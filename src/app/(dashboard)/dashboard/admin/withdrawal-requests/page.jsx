...
export default function WithdrawalRequests() {
    ...
    const fetchWithdrawals = () => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/admin/withdrawals/pending`, {
            headers: { Authorization: `Bearer ${session.user.email}` },
        }).then(r => r.json()).then(data => { setWithdrawals(data || []); setLoading(false); }).catch(() => setLoading(false));
    };

    useEffect(() => { fetchWithdrawals(); }, [session]);
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className={`text-sm text-gray-500 mt-1 ${inter.className}`}>Review and approve or reject withdrawal requests from creators.</p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className={`text-sm text-gray-500 ${inter.className}`}>No pending withdrawal requests.</p>
                    </div>
                ) : (
